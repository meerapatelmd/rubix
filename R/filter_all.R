...
filter_all_grepl <-
        function(data,
                ...
                if (evaluates_to == TRUE) {
                                ...
                                data %>%
                                        dplyr::filter_all(dplyr::any_vars(grepl(grepl_phrase, ., ignore.case = ignore.case) == TRUE))
                        
                } else {
                        data %>%
                                dplyr::filter_all(dplyr::any_vars(grepl(grepl_phrase, ., ignore.case = ignore.case) == FALSE))
                }
        }
